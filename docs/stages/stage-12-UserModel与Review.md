...
- **确认候选重复复审缺陷【真实】**：确认/编辑后的候选不会从候选队列消失——下次审查会再次展示，二次确认会生成重复的持久记录与重复 guard。修复：`handled.json` 处理登记 + `BehaviorRuntime.markHandled()`，候选投影排除已处理项（重启后依旧排除，观察日志新增同候选记录也不复活）。回归测试钉死。
- **已知限制（登记，未修）**：guard 的 `taskRegex` 通道以"进程内最新用户消息"为任务文本——多会话共存于同一进程时会串扰（`AssembleContext` 不携带会话标识，干净修复需要 per-session 注入上下文，登记进 PROGRESS 剩余工作）。
